...
const MY_EMAIL = '2lmf.info@gmail.com'; // <--- OVDJE UPIŠI SVOJ EMAIL
...
function sendDailyReport(data) {
  try {
    ...
    // Koristimo fiksni email definiran na vrhu skripte da izbjegnemo permission greške
    const recipient = MY_EMAIL; 
    
    if (!recipient || recipient.includes('@gmail.com') === false) {
      throw new Error("Email primatelja nije ispravno postavljen u skripti (linija 8).");
    }
...
function createReportHtml(data) {
  ...
  locs.forEach(loc => {
      ...
      rows += `<tr style="background-color: ${statusColor};">
  ...
  return `<!DOCTYPE html><html><head><style>body { font-family: 'Helvetica', sans-serif; color: #1e293b; }.header { background: #111111; color: white; padding: 20px; text-align: center; border-radius: 8px 8px 0 0; }.stats { display: flex; gap: 20px; margin: 20px 0; justify-content: space-around; background: #f8fafc; padding: 15px; border-radius: 8px; }.stat-box { text-align: center; }.stat-val { font-size: 24px; font-weight: bold; color: #E67E22; }.stat-label { font-size: 12px; text-transform: uppercase; color: #64748b; }table { width: 100%; border-collapse: collapse; font-size: 14px; }th { text-align: left; padding: 10px; background: #f1f5f9; color: #475569; font-size: 12px; text-transform: uppercase; }.footer { margin-top: 30px; text-align: center; font-size: 12px; color: #94a3b8; }</style></head><body><div class="header"><h1 style="margin:0;">🦈 SharkTrack</h1><p style="margin:5px 0 0; opacity:0.8;">Dnevni Izvještaj • ${data.date}</p></div><div class="stats"><div class="stat-box"><div class="stat-val">${data.stats.total}</div><div class="stat-label">Lokacija</div></div><div class="stat-box"><div class="stat-val">${data.stats.contacted}</div><div class="stat-label">Kontaktirano</div></div><div class="stat-box"><div class="stat-val">${data.stats.offers}</div><div class="stat-label">Ponuda</div></div></div><h3>Pregled aktivnosti</h3><table><thead><tr><th>Vrijeme</th><th>Tip</th><th>Info / Bilješka</th><th>Status</th><th>Link</th></tr></thead><tbody>${rows}</tbody></table><div class="footer">Generirano putem SharkTrack aplikacije • ${new Date().toLocaleString()}</div></body></html>`;
}
...
// ============================================================
// TEST FUNKCIJA (OBVEZNO POKRENI ZA AUTORIZACIJU)
// ============================================================

function forceAuthAndTest() {
  const res = sendDailyReport({
    date: "AUTORIZACIJA",
    stats: { total: 1, contacted: 0, offers: 0 },
    locations: [{ sat: "00:00", tag: "TEST", contact: "Test", biljeska: "Test autorizacije", mapsLink: "http://maps.com", status: "Nova" }]
  });
